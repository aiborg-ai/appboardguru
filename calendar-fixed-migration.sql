-- Fixed Calendar Migration - No Dependencies
-- Run this in your Supabase SQL Editor

-- 1. Calendar Events Table (standalone, no meetings dependency)
...
BEGIN
  ...
  SELECT id INTO test_user_id FROM auth.users WHERE email = 'test.director@boardguru.ai' LIMIT 1;
  ...
  IF test_user_id IS NOT NULL THEN
    ...
    INSERT INTO calendar_events (
      ...
      event_type, color, category, created_by
    ) VALUES (
      test_user_id, 'Board Meeting Q4 Review', 
      'Quarterly board meeting to review Q4 performance and strategy',
      NOW() + INTERVAL '1 day', NOW() + INTERVAL '1 day' + INTERVAL '2 hours',
      'meeting', '#DC2626', 'Board Meetings', test_user_id
    ) RETURNING id INTO board_meeting_id;
    
    -- Create other events individually
    INSERT INTO calendar_events (
      user_id, title, description, start_datetime, end_datetime, 
      event_type, color, category, created_by
    ) VALUES (
      test_user_id, 'ESG Committee Meeting', 
      ...
      'meeting', '#059669', 'Committee Meetings', test_user_id
    );
    ...
    INSERT INTO calendar_events (
      ...
      test_user_id, 'Team Lunch', 
      'Quarterly team lunch meeting',
      NOW() + INTERVAL '2 days', NOW() + INTERVAL '2 days' + INTERVAL '1 hour',
      'personal', '#10B981', 'Personal', test_user_id
    );
    ...
    INSERT INTO calendar_events (
      ...
      test_user_id, 'Risk Assessment Review', 
      'Monthly risk assessment and mitigation review',
      NOW() + INTERVAL '5 days', NOW() + INTERVAL '5 days' + INTERVAL '90 minutes',
      'meeting', '#F59E0B', 'Risk Management', test_user_id
    );
    
    -- Add attendee to the board meeting
    INSERT INTO calendar_attendees (event_id, user_id, email, role) VALUES
    (board_meeting_id, test_user_id, 'test.director@boardguru.ai', 'organizer');
    
    -- Add reminders for the board meeting
    INSERT INTO calendar_reminders (event_id, user_id, reminder_type, minutes_before) VALUES
    (board_meeting_id, test_user_id, 'email', 1440), -- 24 hours before
    (board_meeting_id, test_user_id, 'in_app', 60);  -- 1 hour before
    
    RAISE NOTICE 'Sample calendar data created successfully for test.director@boardguru.ai';
  ELSE
    RAISE NOTICE 'Test user not found. Sample data not created.';
  END IF;
END $$;