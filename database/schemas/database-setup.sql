-- BoardGuru Database Schema Setup
...
-- Registration Requests: Anyone can insert, only admins can read all
...
-- Board Packs: Approved users can manage their own packs
CREATE POLICY "Approved users can view board packs" ON public.board_packs
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM public.users 
            WHERE users.id = auth.uid() 
...
CREATE POLICY "Approved users can insert board packs" ON public.board_packs
