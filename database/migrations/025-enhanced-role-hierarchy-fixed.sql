-- Migration: Enhanced Role Hierarchy System (FIXED)
-- Agent: DBA-01 (Database Architect)
-- Purpose: Implement 5-level role hierarchy with granular permissions
-- Date: 2025-08-29
-- Fix: Properly handle default values when changing enum types
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        ALTER TABLE users ALTER COLUMN role SET DEFAULT 'pending'::user_role_new;
...
-- Step 4: Add reviewer-specific fields to users table (check if columns exist first)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'users' AND column_name = 'is_reviewer') THEN
        ALTER TABLE users ADD COLUMN is_reviewer BOOLEAN DEFAULT false;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'users' AND column_name = 'reviewer_metadata') THEN
        ALTER TABLE users ADD COLUMN reviewer_metadata JSONB DEFAULT '{}';
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'users' AND column_name = 'test_environment_access') THEN
        ALTER TABLE users ADD COLUMN test_environment_access BOOLEAN DEFAULT false;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'users' AND column_name = 'bug_reports_count') THEN
        ALTER TABLE users ADD COLUMN bug_reports_count INTEGER DEFAULT 0;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'users' AND column_name = 'last_review_activity') THEN
        ALTER TABLE users ADD COLUMN last_review_activity TIMESTAMPTZ;
    END IF;
END $$;
...
-- Director permissions (legacy, mapped to user)
('director', 'boards.access_assigned', '{"allowed": true}', 'Access assigned boards'),
('director', 'meetings.participate', '{"allowed": true}', 'Join and participate in meetings'),
('director', 'voting.cast', '{"allowed": true}', 'Cast votes in decisions'),
('director', 'documents.upload', '{"allowed": true}', 'Upload and share documents'),
('director', 'documents.collaborate', '{"allowed": true}', 'Collaborate on documents'),
('director', 'ai.use_assistants', '{"allowed": true}', 'Access AI assistants'),
('director', 'dashboard.personal', '{"allowed": true}', 'View personal dashboard'),
('director', 'notifications.receive', '{"allowed": true}', 'Receive notifications'),
('director', 'chat.participate', '{"allowed": true}', 'Use board chat features'),
...
DROP POLICY IF EXISTS "Admins and superusers can manage custom permissions" ON user_custom_permissions;
DROP POLICY IF EXISTS "Users can view their own role history" ON role_change_history;
DROP POLICY IF EXISTS "Reviewers can create bug reports" ON bug_reports;
DROP POLICY IF EXISTS "Bug reports are visible to reviewers and admins" ON bug_reports;
...
-- Step 12: Update existing users to map to new roles (only if needed)
UPDATE users 
SET role = 'user'::user_role
WHERE role = 'director'::user_role 
  AND NOT EXISTS (
