  ...
  for (const org of organizations) {
    try {
      ...
      if (orgError) {
        ...
        errors++;
        results.push({ name: org.name, status: 'error', error: orgError.message });
        continue;
      }
      ...
      const { error: memberError } = await supabase
        ...
        .insert({
          ...
          status: 'active',
          invited_by: user.id,
          is_primary: organizations.indexOf(org) === 0  // First org is primary
        });

      if (memberError) {
        console.error('Error adding member for:', org.name, memberError);
        errors++;
        results.push({ name: org.name, status: 'partial', error: memberError.message });
      } else {
        created++;
        results.push({ name: org.name, status: 'success' });
      }
    } catch (error) {
      console.error('Unexpected error for:', org.name, error);
      errors++;
      results.push({ name: org.name, status: 'error', error: String(error) });
    }
  }

  return NextResponse.json({ 
    success: true, 
    message: `Created ${created} organizations, ${errors} errors`,
    created,
    errors,
    results,
    userId: user.id,
